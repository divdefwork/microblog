...
def before_request():
    ...
    g.locale = str(get_locale())  # !from flask_babel import _, get_locale
...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Тепер ваша публікація відображається!')
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Ваші зміни були збережені.')
...
def search():
    ...
    prev_url = url_for('main.search', q=g.search_form.q.data, page=page - 1) \
        if page > 1 else None
    return render_template('search.html', title='Пошукs', posts=posts,
